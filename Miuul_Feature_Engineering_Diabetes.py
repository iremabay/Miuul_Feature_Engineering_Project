...
def load_diabetes_db():
    data = pd.read_csv(r"C:\Users\iremp\Desktop\Miuul ML\diabetes.csv")
    return data
...
#Step 4: Perform target variable analysis. (Mean of the target variable by categorical variables,
#mean of numerical variables by the target variable)
...
#Step 1: Perform necessary operations for missing and outlier values. While there are no missing observations in the dataset,
#instances containing 0 values in variables such as Glucose, Insulin, etc. could potentially represent missing values.
#For instance, a person's glucose or insulin value cannot be 0. Taking this into account,
#you can replace zero values with NaN in the relevant variables and subsequently apply operations for handling missing values.
...
df.loc[(df["BMI"] < 18.5) & (df["Age"] >= 50), "NEW_AGE_BMI_NOM"] = "underweight_senior"
...
#One-Hot Encoding
#The 'outcome' variable should not be in the cat_cols list.
